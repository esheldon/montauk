...
def test_load_example_obsdata(band):
    obsdata = montauk.simtools.load_example_obsdata(band)
...
def test_load_example_instcat(band):
    ...
    cat = montauk.simtools.load_example_instcat(rng=rng, band=band)
...
def test_ccd_radec_generator(n):
    ...
    dm_detector = montauk.camera.make_dm_detector(35)
    obsdata = montauk.simtools.load_example_obsdata(band='i')

    wcs, _ = montauk.wcs.make_batoid_wcs(
        obsdata=obsdata, dm_detector=dm_detector,
    )

    radec_gen = montauk.simtools.CCDRadecGenerator(rng=rng, wcs=wcs)
